...
#' @param alpha Proportion of random true 0.
...
generate_single <- function(data=str1,alpha_0,alpha_1, beta_l,beta_g,beta_m,alpha,n_single){
  ...
  ##true 0 positions
  thresh <- quantile(lambda[upper.tri(lambda)],0.2)
  ...
  subsampling=function(v,alpha)
  {
    ...
    v3=cumsum(v1)-v+1
    total=sum(v1)
    index=sample(1:total,total*alpha,replace=F)
  ...
  if(alpha==0){
    ...
    random0 <- true0rows[sample(1:l,floor(l*alpha))]
